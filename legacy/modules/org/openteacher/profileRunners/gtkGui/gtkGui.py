...
def installGtkClasses():
	...
	class TeachWindow(Gtk.Window):
		def __init__(self, createLessonType, createEvent, compose, createController, *args, **kwargs):
			...
			self._skipButton.connect("clicked", lambda arg: self._controller.skipTriggered())
			self._correctAnywayButton.connect("clicked", lambda arg: self._controller.correctAnywayTriggered())
			...
			self._controller.focusInput.handle(lambda: self._inputEntry.grab_focus())
