...
class TestCase(unittest.TestCase):
	...
	def testLessonDoneCalled(self):
		for mod in self._mods:
			...
			def lessonDone():
				self.assertTrue(lessonType.list['tests'][-1]['finished'])
				if mod not in self._mm.mods('active', type='javaScriptLessonType'):
					# JS cannot change Python objects directly, so exclude
					# it from this check. The one above should handle it
					# fine, anyway...
