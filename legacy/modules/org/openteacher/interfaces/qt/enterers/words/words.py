...
def installQtClasses():
	...
	class WordsTableItemDelegate(QtWidgets.QStyledItemDelegate):
	...
	class WordsTableView(QtWidgets.QTableView):
		...
		def moveCursor(self, cursorAction, modifiers):
			...
			"""
			if cursorAction not in (QtWidgets.QAbstractItemView.MoveNext, QtWidgets.QAbstractItemView.MovePrevious):
		...
		def setData(self, index, value, role=QtCore.Qt.EditRole):
			...
			while True:
				...
				try:
					...
					if index.column() == self.QUESTIONS:
						word["questions"] = self._parse(value)
					elif index.column() == self.ANSWERS:
						word["answers"] = self._parse(value)
					elif index.column() == self.COMMENT:
						word["comment"] = value.strip()
						if not len(word["comment"]):
							del word["comment"]
					elif index.column() == self.COMMENT_AFTER_ANSWERING:
						word["commentAfterAnswering"] = value.strip()
	...
	class EnterWidget(QtWidgets.QSplitter):
		...
		def _buildUi(self, createChecker, keyboardWidget):
			...
			self._titleTextBox = QtWidgets.QLineEdit(self)
			self._questionLanguageTextBox = QtWidgets.QLineEdit(self)
			self._questionLanguageLabel = QtWidgets.QLabel()
			self._answerLanguageTextBox = QtWidgets.QLineEdit(self)
			self._answerLanguageLabel = QtWidgets.QLabel()
			...
			self._removeSelectedRowsButton = QtWidgets.QPushButton(self)
			shortcut = QtWidgets.QShortcut(QtGui.QKeySequence.Delete, self)
