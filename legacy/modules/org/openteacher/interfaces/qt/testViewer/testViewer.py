...
def installQtClasses():
	...
	class TestViewer(QtWidgets.QSplitter):
		def __init__(self, list, test, testTable, calculateNote, progressWidget, *args, **kwargs):
			...
			self.totalThinkingTimeLabel = QtWidgets.QLabel()
			...
			#Horizontal splitter
			factsLayout = QtWidgets.QVBoxLayout()
			self.completedLabel = QtWidgets.QLabel()
			...
			with contextlib.suppress(AttributeError):
				for fact in self._testTable.funFacts:
					if fact[1] is None:
						label = QtWidgets.QLabel("%s<br />-" % fact[0])
					else:
						label = QtWidgets.QLabel("%s<br /><span style=\"font-size: 14px\">%s</span>" % (fact[0], fact[1]))
					factsLayout.addWidget(label, 0, QtCore.Qt.AlignTop)

			factsLayout.addWidget(self.completedLabel)
			factsLayout.addStretch()

			factsWidget = QtWidgets.QWidget()
