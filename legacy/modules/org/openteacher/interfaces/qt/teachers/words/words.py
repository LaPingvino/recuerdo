...
def installQtClasses():
	...
	class TeachSettingsWidget(QtWidgets.QWidget):
		...
		def retranslate(self):
			...
			for key in self._settingsKeys:
				...
				w = self.formLayout.itemAt(i, QtWidgets.QFormLayout.LabelRole).widget()
				w.setText(setting["name"])

				i += 1

	class TeachLessonWidget(QtWidgets.QSplitter):
		def __init__(self, keyboardWidget, *args, **kwargs):
			super(TeachLessonWidget, self).__init__(*args, **kwargs)

			self.changeSettingsButton = QtWidgets.QPushButton()
			self.wordLabel = QtWidgets.QLabel()
			...
			self.commentAfterAnsweringLabel = QtWidgets.QLabel()
			self.commentAfterAnsweringLabel.setWordWrap(True)
			self.nextButton = QtWidgets.QPushButton()
			self.teachTabWidget = QtWidgets.QTabWidget()
			self.progressBar = QtWidgets.QProgressBar()

			self.sideWidget = QtWidgets.QSplitter(QtCore.Qt.Vertical)
	...
	class TeachWidget(QtWidgets.QStackedWidget):
