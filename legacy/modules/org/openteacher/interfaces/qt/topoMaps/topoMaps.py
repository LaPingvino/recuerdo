...
def installQtClasses():
	...
	class Map(QtWidgets.QGraphicsView):
		...
		def __init__(self,*args, **kwargs):
			...
			if QtOpenGL and base._openGlSetting["value"]:
	...
	class TeachPlaceOnMap(QtWidgets.QGraphicsRectItem):
	...
	class EnterMapScene(QtWidgets.QGraphicsScene):
		...
		def mouseDoubleClickEvent(self,gsme):
			...
			if x > 0 and y > 0:
				# Ask for the name
				name = QtWidgets.QInputDialog.getText(self.enterMap, _("Name for this place"), _("What's this place's name?"))
	...
	class EnterMap(Map):
		...
		def __init__(self, enterWidget, *args, **kwargs):
			...
			self.placesList = []
			self.placesGroup = QtWidgets.QGraphicsItemGroup()
		...
		def update(self):
			...
			for place in self.enterWidget.list["items"]:
				...
				rect = QtWidgets.QGraphicsRectItem(place["x"],place["y"],6,6)
				...
				shadow = QtWidgets.QGraphicsTextItem(place["name"])
				...
				item = QtWidgets.QGraphicsTextItem(place["name"])
	...
	class TeachPictureScene(QtWidgets.QGraphicsScene):
		...
		def mouseReleaseEvent(self, event):
			# Clicked a place
			clickedObject = self.itemAt(event.lastScenePos().x(), event.lastScenePos().y(), QtGui.QTransform())
	...
	class TeachPictureMap(Map):
		...
		def setInteractive(self, val):
			if val:
				...
				self.crosshair = QtWidgets.QGraphicsPixmapItem(crosshairPixmap)
